# llm-playground
Repo of me learning llm programming
- langchain
    - pdf-reader-summarizer: Implementaiton of RAG using pdf files as input
    - csv-reader-summarizer: Implementaiton of RAG using csv files as input
    - sql-database-llm: An LLM connected to a database (ON HOLD - Currently have no free plan for openai API)
    - langchain-integrations: Implementation of valrious langchain integrations
    - mcp-implementation: Implementation of the MCP Framework for Agents
    - rag-multiple-data-sources: A RAG implementation that uses both csv and pdf files
- langgraph
    - first-project: Used Sample Code from the langgraph docs
    - graph-exercise: Modified sample code from langgraph docs to make an agent that sends emails and text messages